...
class MPCController(IController):
    ...
    Model: x_dot = A x + B u
    where x = [position; velocity], u = acceleration command
    
    At each step, solves:
    min sum_{k=0}^{N-1} [||x_k - x_ref||^2_Q + ||u_k||^2_R]
    subject to:
        x_{k+1} = A_d x_k + B_d u_k  (discrete dynamics)
        u_min <= u_k <= u_max         (control constraints)
    
    Returns first control action u_0.
    """
    ...
    def _discretize(self, dt: float):
        ...
        n = self.A_cont.shape[0]
        I = np.eye(n)
        
        # Matrix exponential approach for discretization
        # For A = [[0,1],[0,0]], we can compute analytically
        # A_d = [[1, dt], [0, 1]]
        # B_d = [[dt²/2], [dt]]
    ...
    def _predict_trajectory(self, x0: np.ndarray, u_sequence: np.ndarray, dt: float) -> np.ndarray:
        ...
        for k in range(N):
            x_traj[k + 1] = self.A_d @ x_traj[k] + self.B_d.flatten() * u_sequence[k]
